...
ms.locfileid: "68271399"
---
# <a name="troubleshoot-gateways---power-bi"></a>对网关进行排除故障 - Power BI

[!INCLUDE [gateway-rewrite](includes/gateway-rewrite.md)]

本文介绍将本地数据网关用于 Power BI 时的一些常见问题  。 如果遇到下文未列出的问题，可以访问 Power BI [社区](http://community.powerbi.com)网站，或者可以创建[支持票证](http://powerbi.microsoft.com/support)。
...
配置结束时，将再次调用 Power BI 服务以验证网关。 Power BI 服务没有将网关报告为动态。 重启 Windows 服务可能会使通信成功。 你可以收集并查看日志以获取更多详细信息，如[收集本地数据网关应用的日志](/data-integration/gateway/service-gateway-tshoot#collect-logs-from-the-on-premises-data-gateway-app)中所述。
...
请确保已按[添加数据源](service-gateway-data-sources.md#add-a-data-source)中所述，向网关添加一个或多个数据源。 如果“管理网关”下的管理门户中未显示网关，请尝试清除浏览器缓存或注销服务然后重新登录  。
...
2. 贵组织内的任何网关上均没有可用的数据源。 可以在新的或现有的本地数据网关上配置数据源。
...
如果你的 Azure Active Directory UPN 与本地 Active Directory UPN 不匹配，则可以使用[映射用户名](service-gateway-enterprise-manage-ssas.md#mapping-usernames-for-analysis-services-data-sources)功能将其替换为有效的值。 或者，可以通过租户管理员或本地 Active Directory 管理员更改 UPN。

## <a name="kerberos"></a>Kerberos

如果基础数据库服务器和本地数据网关未针对 [Kerberos 约束委派](service-gateway-sso-kerberos.md)进行适当配置，则在网关上启用[详细日志记录](/data-integration/gateway/service-gateway-performance#slow-performing-queries)，并基于网关日志文件中的错误/跟踪进行调查，作为故障排除的起始点。 若要收集网关日志以供查看，请参阅[收集本地数据网关应用的日志](/data-integration/gateway/service-gateway-tshoot#collect-logs-from-the-on-premises-data-gateway-app)。
...
如果使用 UPN (alias@domain.com) 模拟用户，那么在 SAP HANA 中配置的外部 ID 与登录名不匹配时，就会收到 1033 错误。 在日志中，你将在错误日志顶部看到“原始 UPN ‘alias@domain.com’已替换为新 UPN ‘alias@domain.com’”，如下所示。
...
在计划刷新中使用网关时，如果需要创建一个支持请求，刷新历史记录可帮助查看发生了什么错误，以及提供有用的数据  。 可以查看计划刷新和按需刷新。 以下步骤介绍如何访问“刷新历史记录”  。
