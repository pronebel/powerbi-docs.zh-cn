...
此设置适用于数据刷新和报表呈现。 数据刷新同时执行数据源中的数据刷新和查询刷新，除非禁用查询刷新。 如果未禁用查询刷新，则此内存限制还适用于这些查询。 即使数据刷新成功，任何失败的查询也会导致将计划的刷新状态报告为失败。

#### <a name="query-timeout"></a>查询超时

使用此设置可以更好地控制长时间运行的查询，此类查询可能会导致用户加载报表速度缓慢。 此设置适用于数据刷新和报表呈现。 数据刷新同时执行数据源中的数据刷新和查询刷新，除非禁用查询刷新。 如果未禁用查询刷新，则此超时限制还适用于这些查询。
