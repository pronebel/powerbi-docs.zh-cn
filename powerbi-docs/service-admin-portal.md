...
Azure Active Directory B2B 来宾用户可以编辑和管理组织中的内容。 [了解详细信息](service-admin-azure-ad-b2b.md)
...
> [!IMPORTANT]
> 将外部用户添加到 Azure AD 租户（手动或通过与该用户共享报表）时，内部帐户现在可以发现该用户帐户。 即使内部帐户没有外部共享权限，内部用户也可以与外部用户共享其他内容。
