...
5. 如果对应用程序类型选择“服务器端 Web 应用程序”，然后继续输入“主页 URL”和“重定向 URL”的值    。 **重定向 URL**可与任何有效的 URL，并应与已创建的应用程序相对应。 如果已选择“本机”，则转至步骤 6  。
...
7. 如果愿意**本机**为应用程序类型，然后将为你然后提供**应用程序 ID**。 如果对应用程序类型选择“服务器端 Web 应用”，则系统会提供“应用程序 ID”和“应用程序密码”    。
...
5. 按照提示进行操作，并创建新的应用程序。
   ...
   有关如何在 Azure Active Directory 中注册应用程序的详细信息，请参阅[与 Azure Active Directory 注册应用](https://docs.microsoft.com/azure/active-directory/develop/quickstart-v2-register-an-app)
...
3. 内**API 的权限**，选择**添加权限**，然后选择**Power BI 服务**。

    ![应用权限 03](media/register-app/powerbi-embedded-azuread-app-permissions03.png)

4. 选择下所需要的特定权限**委托的权限**。 逐一选中这些选项以保存所做的选择。 完成时选择“保存”  。

5. 选择**授予同意**。

    **授予同意**操作所需*主帐户*由 Azure AD 的同意提示。 如果执行此操作的帐户是全局管理员，会向组织内此应用的所有用户授予权限。 如果执行此操作的帐户是主帐户，而不是全局管理员，将仅向此应用程序的主帐户授予权限   。
...
5. 向 Azure Active Directory (AAD) 授予应用权限
   ...
   * **AllPrincipals**只能由租户管理员的租户中的所有用户授予权限。
   * **主体**用来授予特定用户的权限。 在此情况下，应将附加属性添加到请求正文 - principalId={User_ObjectId}  。
