...
* **将 SAP HANA 视为多维源（默认）：** 在这种情况下，该行为将类似于 Power BI 连接到其他多维源（如 SAP Business Warehouse 或 Analysis Services）时的行为。 连接到 SAP HANA 使用此设置，单个分析或计算选择视图和所有度量值时，层次结构和该视图的属性将可在字段列表中。 创建视觉对象后，将始终从 SAP HANA 中检索聚合数据。 建议使用此方法，且 SAP HANA 中的新 DirectQuery 报表均默认采用此方法。

* **将 SAP HANA 视为关系源：** 此情况下，Power BI 将 SAP HANA 视为关系源。 这提供了更大的灵活性。 必须使用此方法时格外小心，以确保按预期方式聚合度量值并避免出现性能问题。

连接方法由全局工具选项，通过选择设置**文件 > 选项和设置**，然后**选项 > DirectQuery**，然后选择选项**将 SAP HANA 视为关系源**下, 图中所示。 

![](media/desktop-directquery-sap-hana/directquery-sap-hana_01a.png)

通过“将 SAP HANA 视为关系源”选项，可控制将 DirectQuery 用于 SAP HANA 的所有新报告所采用的方式  。 该选项不影响当前报表中的任何现有 SAP HANA 连接，也不影响打开的任何其他报表中的连接。 因此，如果当前取消选中该选项，则在使用“获取数据”向 SAP HANA 添加新连接时，该连接会将 HANA 视为多维源  。 但是，如果打开的其他报表的同时连接到 SAP HANA，则该报表将继续根据设置的选项的行为*在创建的时*，这意味着，连接到 SAP HANA 的任何报表2018 年 2 月之前创建将继续将 SAP HANA 视为关系源。 

两种方法构成不同的行为，并不能切换到另一种方法从现有报表。 
...
连接到 SAP HANA 视为多维源时，应该注意以下事项：

* 在“获取数据导航器”中，可选择单个 SAP HANA 视图  。 不能选择各个度量值或属性。 在连接时没有定义查询，这与数据导入操作不同，也与在将 SAP HANA 视为关系源的情况下使用 DirectQuery 时的操作不同。 这也意味着，不能直接使用 SAP HANA SQL 查询时选择此连接方法。
...
* 在 SAP HANA 中，可定义一个属性，它将采用其他属性作为其标签。 例如，**产品**（使用值 1、 2、 3 等） 可以使用**ProductName** (与值 Bike、 Shirt、 Gloves，等) 作为其标签。 在这种情况下，单个字段 Product  将显示在字段列表中，其值是标签 Bike、Shirt、Gloves 等，但它们将按键值 1、2、3 进行排序并由键值决定其唯一性。 另外，还会创建一个隐藏列 Product.Key  ，从而允许根据需要访问基础键值。 

连接时会显示基础 SAP HANA 视图中定义的所有变量，还可输入必要的值。 可以随后更改这些值，通过选择**编辑查询**从功能区，然后**管理参数**从显示的下拉列表菜单。 
...
* 在视觉对象中，任何后续聚合（如 Sum  、Average  、Min  等）都在该导入的数据上执行。 例如，在视觉对象中包含 AveragePrice  时，将默认使用 Sum  聚合，并针对每个 ProductID  在 AveragePrice  上返回总和 - 本例中为 13.67。 这同样适用于视觉对象中使用的任何替代聚合函数（如 Min  、Average  等）。 例如，*平均*的*AveragePrice*返回 6.66、 4 和 3，即等于 4.56 的平均值和的平均值不*价格*六个记录中的基础表，该表是 5.17。
...
如果在**获取数据**体验中，所做的选择是针对**ProductID**并**AveragePrice**度量值，那么即对请求，该视图定义的查询聚合数据 （在前面的示例中，为简单起见伪 SQL，使用 SAP HANA SQL 的确切语法不匹配）。 视觉对象中定义的任何更深入聚合都将进一步聚合此类查询的结果。 如上文针对 SQL Server 所述，这同时适用于导入和 DirectQuery 用例。 在 DirectQuery 情况下，将查询从**获取数据**或**查询编辑器**将发送到 SAP HANA 的单个查询中的嵌套 select 语句中使用，因此它不是实际的用例，将在之前读取所有数据为进一步聚合。  

由于上述所有考虑事项和行为，因此在通过 SAP HANA 使用 DirectQuery 时需注意以下重要事项：  

* 每当 SAP HANA 中的度量值并非累加时（例如不是简单的 Sum、Min 或 Max），必须注意在视觉对象中执行的任何进一步聚合    。

* 在“获取数据”或“查询编辑器”中，只能包含所需的列以检索所需数据，这表示结果将为一个查询，而该查询必须是可发送到 SAP HANA 的合理查询   。 例如，如果已选择许多列，认为它们可能需要在后续视觉对象，然后甚至对于 DirectQuery 的简单视觉将意味着嵌套 select 语句中使用的聚合查询将包含这些列，通常将执行效果不佳。
  
我们来看一个示例。 下例中，在“获取数据”对话框中选择五个列（CalendarQuarter、Color、LastName、ProductLine、SalesOrderNumber），还选择度量值 OrderQuantity，该操作意味着以后创建包含 Min OrderQuantity 的简单视觉对象时会将以下 SQL 查询发送到 SAP HANA        。 阴影是嵌套 select 语句，其中包含“获取数据”   / “查询编辑器”  中的查询。 如果此嵌套 select 语句提供高的基数结果，然后生成 SAP HANA 性能可能会很差。  
...
* **使用 SSL 连接**-可以使用导入连接并使用具有 SSL 多维度，购买不能连接到 SAP HANA 实例配置为适用于关系连接器使用 SSL。
