...
自定义视觉对象是包含用于呈现数据代码的包来为数据提供自定义视觉服务。 任何人都可以创建自定义视觉对象并将其打包为可导入 Power BI 报表的单个 `.pbiviz` 文件。
...
### <a name="import-organization-visuals"></a>导入组织级视觉对象
...
5. 单击自定义视觉对象下载链接。
    ...
    下载页还介绍了如何将自定义视觉对象导入到 Power BI Desktop 和 Power BI 服务中。
