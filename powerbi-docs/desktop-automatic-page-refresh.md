...
容量 是一个 Power BI 核心概念，表示用于托管和交付 Power BI 内容的一组资源（存储、处理器和内存）。 容量可以是共享容量，也可以是专用容量。 共享容量与其他 Microsoft 客户共享，而专用容量则完全委派给单个客户。 [高级容量](whitepaper-powerbi-premium-deployment.md#capacities)一文中介绍了专用容量。
...
1. 所有自动页面刷新查询都以较低的优先级运行，以确保交互式查询（如页面负载和交叉筛选视觉对象）优先。
