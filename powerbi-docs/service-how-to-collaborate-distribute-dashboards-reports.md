...
无论选择了哪个选项，要共享仪表板，都需要 [Power BI Pro 许可证](service-features-license-type.md)，或者该内容需要位于[高级容量](service-premium-what-is.md)中。 许可证要求因查看仪表板的同事而各不相同，具体取决于所选择的选项。 以下各节将进行详细说明。 
...
当团队协同工作时，他们需要访问相同的文档，以便快速协作。 在 Power BI 工作区中的团队一起共享的所有权和管理其仪表板、 报表、 数据集和工作簿。 Power BI 用户组有时根据组织结构组织其工作区，而在其他时候为特定项目创建工作区。 仍有一些组织使用多个工作区来存储所用报表或仪表板的不同版本。 
...
- 如果同事需要保持最新的仪表板或需要访问工作区中的所有内容，请考虑将它们添加到工作区。 
- 如果同事只需查看该仪表板和工作区中的不是所有内容，您再次必须替代项。 
- 如果仪表板是需要分发给许多同事的众多内容的一部分，那么发布应用可能是最好的选择。 
- 但是，如果同事只需要一个仪表板，然后共享仪表板可能是最佳解决方案。

详细了解[创建工作区](service-create-workspaces.md)。

**你知道吗？** Power BI 的新工作区体验。 读取[创建新工作区](service-create-the-new-workspaces.md)若要查看工作区如何变化。 
...
- **在 SharePoint Online 中嵌入**适用于 Power BI web 部件的 SharePoint Online。 它控制如何嵌入在报表中提供单一登录体验。 
- **嵌入**适用于任何门户或支持使用的 URL 或 iFrame 的嵌入内容的网站。 

无论选择哪个选项，Power BI 之前强制执行所有权限和数据安全性的用户可以看到的内容。 查看报告的人员需要具有相应许可证。 详细了解[在 SharePoint Online 中嵌入](service-embed-report-spo.md)并[嵌入](service-embed-secure.md)Power BI 中的选项。
