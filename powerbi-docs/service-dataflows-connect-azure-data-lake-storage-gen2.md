...
* 数据流的数据和定义文件便可供您的 IT 部门的开发人员可以使用 Azure 数据和人工智能 (AI) 服务中所示[从 Azure 数据服务的 GitHub 示例](https://aka.ms/cdmadstutorial)
* 使你的组织中的开发人员能够将数据流数据集成到内部应用程序和数据流和 Azure 中使用开发人员资源的业务线解决方案
...
* **Azure 存储帐户已启用的数据湖存储第 2 代功能** 

> [!TIP]
> 如果没有 Azure 订阅，请在开始之前先创建一个[免费帐户](https://azure.microsoft.com/free/)。

> [!WARNING]
> 数据流存储位置一旦配置即无法更改。 请参阅[注意事项和限制](#considerations-and-limitations)附近其他要考虑的重要元素本文末尾部分。
...
> [!NOTE]
> 至少留出 30 分钟，权限将传播到 Power BI 从门户。 每当您更改权限在门户中，允许 30 分钟，在 Power BI 中反映这些权限。 
...
5. 选择并从你的搜索结果中复制的两个 Power BI 服务的对象 Id 和联机 Power Query。 准备好在后续步骤中粘贴这些值。
...
一旦你已设置 Azure 数据湖存储第 2 代帐户在 Azure 门户中，您将其连接到中的 Power BI **Power BI 管理门户**。 你还管理 Power BI 中的数据流存储**数据流存储**的 Power BI 管理门户的设置部分。 有关启动和基本使用的指南，请参阅[如何访问管理门户](service-admin-portal.md)，获取详细信息。

使用以下步骤连接  Azure Data Lake Storage Gen2 帐户：

1. 导航到**数据流设置**选项卡**Power BI 管理门户**

    ![Power BI 管理门户](media/service-dataflows-connect-azure-data-lake-storage-gen2/dataflows-connect-08b.png) 

2. 选择**连接在 Azure 数据湖存储第 2 代**按钮。 将显示以下窗口。
...
若要授予存储分配权限，请转到**数据流设置**选项卡**Power BI 管理门户**。 存在  “允许工作区管理员将工作区分配给此存储帐户”单选按钮，该按钮必须设置为  “允许”。 启用该滑块后，选择  “应用”按钮以使更改生效。 
...
3. Ben 会收到类似的错误，因为他未授权到 lake 中的数据流的 CDM 文件夹。
