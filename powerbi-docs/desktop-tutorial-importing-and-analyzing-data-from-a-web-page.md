...
- 连接到 Web 数据源并导航访问到可用表，
- 在Power Query 编辑器中调整并转换数据，
...
2. 在“从 Web”对话框中，将 URL `http://en.wikipedia.org/wiki/UEFA_European_Football_Championship` 粘贴到“URL”文本框，然后选择“确定”。
   ...
   在连接到维基百科网页后，Power BI“导航器”对话框会在页面上显示可用的表单。 可以选择任意表名称来预览其数据。 虽然“结果[编辑]”表具有所需的数据，但是它不完全符合你希望的数据内容。 你可以先重新修整并清理数据，然后再将其加载到报表中。 
...
3. 在“导航器”列表中选择“结果[编辑]”表，然后选择“编辑”。 
   ...
   在“Power Query 编辑器”中将打开表的预览信息，你可以在其中应用转换以清理数据。 
...
你希望通过仅显示年份和获胜的国家/地区来使数据更易于扫描。 你可以使用“Power Query 编辑器”执行这些数据修整和清理步骤。
...
某些“年份”单元格仅包含单词“年份”而不是年份值。 你可以筛选“年份”列来删除只包含“年份”一词的行。 
...
既然已按所需的方式修整完数据，你可以准备好将查询重命名为“欧洲杯获胜队”并将其导入报表。
...
1. 在“字段”窗格中选择“国家/地区”字段，或者将其拖到报表画布上。 Power BI Desktop 会将数据识别为国家/地区名称，并自动创建“地图”可视化效果。 
...
3. 地图针对每个欧洲杯锦标赛获胜国家/地区显示相同大小的数据点。 若要使每个数据点的大小反映国家/地区的获胜频率，可将“年份”字段拖动到“可视化效果”窗格下半部分“大小”处的位置。 字段自动更改为“年份计数”度量值，地图可视化效果现在针对锦标赛获胜次数较多的国家/地区会显示较大数据点。 
...
可以更改可视化效果的外观，方法是在“可视化效果”窗格中选中它，然后选择“格式”（绘制滚刷）图标。 例如，可视化效果中的“德国”数据点具有误导性，因为西德获胜两次锦标赛而德国获胜一次，地图重叠显示了这两个点而不是将它们分隔或统一计算。 你可以分别为这两个点添加颜色，以突出区分它们。 你还可以为地图提供一个更具描述性和吸引力的标题。 
