---
title: 在 Power BI 中的高密度行采样
description: 在 Power BI 中的高密度行采样
...
---
# <a name="high-density-line-sampling-in-power-bi"></a>在 Power BI 中的高密度行采样
从 2017 年 6 月发布 Power BI Desktop 和 Power BI 服务更新开始，可使用新的采样算法来改进对高密度数据进行采样的视觉对象   。 例如，每个零售店每年的销售额超过一万，可以根据零售店的销售业绩创建一个折线图。 此类销售信息折线图将从每个存储的数据中采样的数据 （选择有意义的代表该数据，来说明如何随时间而改变销售），并创建多系列折线图表示基础数据。 这是将高密度数据可视化的常见做法。 Power BI Desktop 改进了高密度数据的采样，本文详细介绍了相关信息。
...
## <a name="how-high-density-line-sampling-works"></a>如何高密度行采样的工作方式
以前，Power BI 以确定性方式在所有基础数据中选择采样数据点的集合  。 例如，对于视觉对象跨越一个日历年的高密度数据，视觉对象中可能会显示 350 个示例数据点，选择每个数据点才能确保视觉对象中呈现出所有数据（基础数据的整体系列）。 为了帮助了解此操作，假设在一年期内，绘制股票价格并选择 365 数据点来创建折线图视觉对象 （对应一个数据点的每一天）。
...
对于高密度视觉对象**Power BI**以智能方式到高分辨率的区块，切片数据，然后选取重要的点来表示每个区块。 对高分辨率的数据进行切片的过程经过专门优化，以确保生成的图表在外观上区别呈现的所有基础数据点，但速度更快、 更具交互性。

### <a name="minimum-and-maximum-values-for-high-density-line-visuals"></a>最小和最大值的高密度行视觉对象
以下限制适用于任何给定的视觉对象：
...
让我们假设你要创建视觉对象根据股票价格，并且比较两个不同的股票，这两种使用**High-Density 采样**。 每个系列的基础数据都有大量数据点（也许一天中每秒都在捕获股票价格）。 高密度行采样算法会对每个系列执行独立分箱。
...
默认情况下，新算法处于“开启”状态  。 若要更改此设置，请转到**格式设置**窗格中，在**常规**卡以及底部，请参阅名为的切换滑块**High-Density 采样**。 若要将其关闭，将滑块滑动到“关闭”  。
...
* 由于粒度增加和装箱过程，“工具提示”只能在代表数据与光标对齐时显示值  。 请参阅*工具提示和高密度行采样*中此文章了解详细信息部分。
