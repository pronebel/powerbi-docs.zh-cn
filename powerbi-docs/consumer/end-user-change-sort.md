...
无法对仪表板上的视觉对象进行排序。 但在 Power BI 报表中，你可以一次根据一个字段（有时两个）对大多数可视化效果进行排序。 对于某些类型的视觉对象，排序不可用：树状图、仪表、地图等。 
...
![显示用于排序的第一列的屏幕截图](media/end-user-change-sort/power-bi-sort-first.png)
...
如果在同一列上再次按 Shift 并单击，则会更改该列的排序方向。 此外，如果按 Shift 并单击之前已添加到排序顺序的列，则会将该列移到排序顺序后面。
