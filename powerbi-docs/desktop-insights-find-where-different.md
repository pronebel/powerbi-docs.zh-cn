...
对于简单的累加性度量值，例如此示例中的*总销售额*，将根据相对值而非绝对值进行比较。 因此尽管 Touring bikes 销售额低于所有类别的总销售额，默认情况下视觉对象使用双轴以允许跨不同国家/地区，而不是所有类别的自行车 Touring bikes 销售额的比例之间的比较。  通过切换视觉对象下方的切换按钮，可在同一轴上显示两个值，从而可以轻松地比较绝对值（如下图所示）。    
...
您可能想知道什么*不同*意味着。 例如，假设美国与加拿大的销售额总体分配如下：
...
因此，对于累加性度量值，如上一示例中的*销售额*，应使用行列图，图中采用双轴和适当的缩放比例，以便轻松比较相对值。 列显示应用了筛选器的值，行显示总体值（正常情况下，列轴位于左侧，行轴位于右侧）。 使用显示的一行*单步执行*样式，为虚线，填入灰色。 上一示例中，如果列轴最大值为 4，和行轴的最大值为 20，则它将允许轻松筛选和整体值之间美国和加拿大相对值的比较。 
...
在检查完各个列以及这些列的值之后，选择提供最大差异的值集。 为了便于理解，随后按列输出这些列/值，其值提供最大差异的列最先列出。 最多三个值将显示每个列，但没有产生很大影响的少于三个值或如果某些值为比其他更有利的决策可能显示更少。 
