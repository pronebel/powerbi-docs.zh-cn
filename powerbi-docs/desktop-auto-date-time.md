...
ms.locfileid: "73878780"
---
# <a name="auto-datetime-in-power-bi-desktop"></a>Power BI Desktop 中的自动日期/时间

本文面向在 Power BI Desktop 中开发导入或复合模型的数据建模人员。

## <a name="background"></a>背景

自动日期/时间是 Power BI Desktop 中的数据加载选项  。 此选项用于实现基于模型中所加载日期列的时间智能报告。 具体而言，它允许报表作者使用日历时间段进行筛选、分组和向下钻取，而无需建模人员进行显式开发。 日历时间段包括年份、季度、月份和日期。
...
> [!NOTE]
> 列名称和值按[模型语言](supported-languages-countries-regions.md#choose-the-language-for-the-model-in-power-bi-desktop)进行转换和格式化。

还会创建自动日期/时间表的“日期”列与模型日期列的关系  。

自动日期/时间表中会加载包含模型日期列中存储的所有日期值的的全部日历年份。 例如，如果日期列中最早的值是 2016 年 3 月 20 日，最新值是 2019 年 10月 23 日，则表将包含 1,461 行。 2016 到 2019 这四个日历年的每个日期对应一行。 刷新模型时，也会刷新每个自动日期/时间表，以确保其始终可包含日期列值。

如果可以查看自动日期/时间表的行，这些行可能如下所示：
...
如果可以在“模型”视图中查看自动日期/时间表，它可能如下所示（相关列已突出显示）：
...
存在包含（可见）日期列的自动日期/时间表时，报表作者不会看到该列显示为“字段”窗格中的字段  。 他们会看到与日期列同名的可扩展对象。 由于它带有日历图标，因此可以轻松识别。 报表作者展开日历对象时，会看到名为“日期层次结构”的层次结构  。 展开层次结构后，会看到以下四个级别：年份、季度、月份和日期     。
...
不过，有一个已添加的功能不受普通层次结构的支持。 将自动日期/时间层次结构（或层次结构的级别）添加到视觉对象框时，报表作者可以在使用层次结构或日期列之间切换。 对于某些视觉对象，如果你只需要日期列而不是层次结构和级别，则这种方法非常有意义。 首先配置视觉对象字段（右键单击视觉对象字段，或单击向下箭头），然后使用上下文菜单在日期列或日期层次结构之间切换。
...
最后，使用 DAX 编写的模型计算可以直接引用日期列，也可以间接引用隐藏的自动日期/时间表列。
...
在 Power BI Desktop 中，选择“文件”>“选项和设置”>“选项”，然后选择“全局”或“当前文件”页    。 在任一页上，都可在“时间智能”部分下方找到该选项  。
