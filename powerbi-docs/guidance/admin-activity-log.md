...
- PowerShell 脚本的用户必须使用 [Connect-PowerBIServiceAccount cmdlet](/powershell/module/microsoftpowerbimgmt.profile/connect-powerbiserviceaccount) 登录，并在看到提示时输入 Power BI 租户管理员凭据。 需要租户管理员特权才能使用活动日志 API。

> [!IMPORTANT]
> 没有租户管理员特权的用户将无法执行示例脚本中的代码片段。
