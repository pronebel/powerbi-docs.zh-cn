...
如果 RDL 报表需要从本地数据源检索数据，则它们不能使用单一登录 (SSO)。 目前，将使用网关数据源用户帐户的安全上下文来从这些源检索所有数据。 SQL Server Analysis Services (SSAS) 不可能基于每个用户强制执行行级别安全性 (RLS)。
...
- Guy in a Cube 视频：[Power BI 中的分页报表简介](https://www.youtube.com/watch?v=wfqn45XNK3M)
