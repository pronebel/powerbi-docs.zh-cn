...
类型 2 SCD 支持维度成员的版本控制。 如果源系统未存储版本，则通常是检测更改的数据仓库加载过程，并适当地管理维度表中的更改。 在这种情况下，维度表必须使用代理键来提供对维度成员的某个版本的唯一引用。 它还包括定义版本的日期范围有效期的列（例如“开始日期”和“结束日期”），并可能包含标记列（例如“IsCurrent”），以便轻松地按当前维度成员进行筛选。
