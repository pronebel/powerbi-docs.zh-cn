...
- **并发活动模型**:当不同的导入模型保持在内存中时, 并发查询会提供最佳的响应能力和性能。 应提供足够的内存来托管所有查询量大的模型，并提供额外的内存来允许刷新。
- **导入模型刷新**:Power Query 查询和计算表/列逻辑的刷新类型 (完整或增量)、持续时间和复杂性可能会影响内存, 尤其是处理器使用情况。 并发刷新受到容量大小（1.5 x 后端 vCore，已向上舍入）的限制。
- **并发查询**:当处理器或 LC/DQ 连接超出容量限制时, 许多并发查询可能会导致无响应的报表。 对于包含许多视觉对象的报表页而言尤其如此。
...
测试是可重复的, 这一点很重要:应运行几次测试，每次得出的结果应大致相同。 这些结果的平均值可用于推断和估算真实生产条件下的工作负载。
