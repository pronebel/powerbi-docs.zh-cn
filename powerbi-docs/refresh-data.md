...
配置完刷新计划后，数据集设置页面会通知你下一个刷新时间，如上面的屏幕截图所示。 如果要加快数据刷新速度以执行网关和数据源配置测试等操作，请使用左侧导航窗格的数据集菜单中的“立即刷新”选项进行按需刷新  。 按需刷新不会影响下一个计划刷新时间，但它们会计入每日刷新限制，如本文前面所述。
