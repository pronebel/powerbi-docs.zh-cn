...
使用 DirectQuery 时，复合模型可以创建 Power BI 模型（例如一个 .pbix Power BI Desktop 文件），它可以执行以下两项操作中的一项或全部：
...
以下示例显示维度表的一种常见用例：例如通过从其他位置导入的额外数据扩展的“Product”或“Customer”。 还有可能使表借助 DirectQuery 连接到各个源。 若要继续了解示例，假定每个“Country”和“Period”的“Sales Targets”都存储在一个单独的部门数据库中。 可以像往常那样使用 GetData 连接到该数据，如下图所示: 
