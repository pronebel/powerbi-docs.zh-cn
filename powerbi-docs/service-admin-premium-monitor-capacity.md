...
| DirectQuery/实时最长发生时间 | DirectQuery/实时连接在一小时内超过 80% 的次数最多的时候的当地时间。 |
| 刷新总计 | 过去七天内的刷新总次数。 |
...
| 每小时刷新计数和内存消耗(GB) |  成功、失败和内存占用率，按一小时的时间段划分，以本地时间报告。 |
| 每小时平均刷新等待时间（分钟） |  平均刷新等待时间，按一小时的时间段划分，以本地时间报告。 多个高刷新等待时间峰值表示容量过度运行。 |
|  |  |
...
| DirectQuery/实时连接（> 80% 利用率） |  DirectQuery 或实时连接超过 CPU 利用率的 80% 的次数，按一小时的时间段划分，以本地时间报告。 |
|  |  |
...
| 每小时查询等待时间分布 |  按一小时的时间段划分的查询等待计数和平均等待时间（以毫秒为单位）与内存占用率（以 GB 为单位），以本地时间报告。 |
|  |  |
...
| 每小时结果 |  成功、失败和内存占用率，按一小时的时间段划分，以本地时间报告。 |
